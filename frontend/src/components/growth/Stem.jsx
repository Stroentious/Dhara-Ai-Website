...
const STEM_VERTEX_SHADER = /* glsl */`
  ...
  void main() {
    ...
    // ── 2. LONGITUDINAL TRUNK FLUTING & BRAIDED ANATOMICAL RIBS ──
    if (uMaturity > 0.08) {
...
const STEM_FRAGMENT_SHADER = /* glsl */`
  ...
  void main() {
    ...
    // ── 3. DIRECTIONAL SUN & AMBIENT ILLUMINATION ──
    vec3 sunDir = normalize(vec3(0.55, 0.85, 0.40));
    vec3 viewDir = normalize(cameraPosition - vWorldPos);

    // Half-Lambert diffuse lighting across bark ridges
    ...
    float cavityAO = clamp(fissureMask * 0.75 + 0.25, 0.0, 1.0);
    ...
    float plateId = vor.z;
    vec3 plateTonalShift = vec3(plateId * 0.07 - 0.035, plateId * 0.05 - 0.025, plateId * 0.03 - 0.015);
    barkColor += plateTonalShift * fissureMask;

    // Apply lighting
    vec3 litBark = barkColor * (diffuse * 0.76 + 0.24);
    // Sun-facing ridges receive warm grazing light
    litBark += uBarkSunlit * max(0.0, NdotL) * plateCrest * 0.20;

    // ── 5. DRY WEATHERED SPECULAR & GRAZING SHEEN ──
    vec3 halfDir = normalize(sunDir + viewDir);
    float NdotH = max(0.0, dot(bumpedNormal, halfDir));
    // Soft, non-plastic diffuse sheen on dry weathered plate ridges
    float specular = pow(NdotH, 14.0) * 0.12 * fissureMask;
    litBark += vec3(0.94, 0.92, 0.88) * specular;

    // Subtle rim lighting for depth separation
    float rim = pow(1.0 - max(0.0, dot(bumpedNormal, viewDir)), 3.2) * 0.12 * fissureMask;
    litBark += uBarkSunlit * rim;

    // ── 6. SELECTIVE NATURAL GREEN MOSS & LICHEN PATINA ──
    // Selective growth in deep moist crevices, shaded orientations, and lower trunk
    float mossInCrevice = creviceDeep * (1.0 - diffuse * 0.6);
    float mossAtBase = smoothstep(0.48, 0.0, vHeightProg);
    float mossNoise = sin(vWorldPos.x * 14.0 + vWorldPos.z * 14.0) * 0.5 + 0.5;
    float mossIntensity = clamp((mossInCrevice * 0.72 + mossAtBase * 0.48) * mossNoise, 0.0, 1.0);

    vec3 botanicalMoss = mix(uMossColor, vec3(0.36, 0.56, 0.22), diffuse * 0.5);
    litBark = mix(litBark, botanicalMoss * (diffuse * 0.65 + 0.35), mossIntensity * 0.68 * uMaturity);

    // ── 7. TRANSITION: Young shoot (tender green) -> Mature Weathered Banyan Bark ──
    float greenFactor = (1.0 - uMaturity * 0.95) + (vHeightProg * 0.16 * (1.0 - uMaturity * 0.8));
    greenFactor = clamp(greenFactor, 0.0, 1.0);

    vec3 finalCol = mix(litBark, uShootColor, greenFactor);

    // ── 8. SAP NUTRIENT PULSE (Early seedling phase) ──
    float sapPulse = sin(-uTime * 2.5 + vHeightProg * 16.0) * 0.5 + 0.5;
    vec3 nutrientGlow = vec3(0.20, 0.90, 0.40) * sapPulse * (1.0 - uMaturity * 0.85) * 0.25;
    finalCol += nutrientGlow;
  ...
  const { trunkGeometries, stemMaterial } = useMemo(() => {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uShootColor: { value: new THREE.Color('#38b556') },
        // Rich Earthy Living Bark Palette
        uBarkDeep: { value: new THREE.Color('#120c08') },     // Deep fissure shadow
        uBarkDark: { value: new THREE.Color('#2c1c13') },     // Sub-bark raw umber
        uBarkMid: { value: new THREE.Color('#523a27') },      // Rich warm walnut bark
        uBarkWarm: { value: new THREE.Color('#735338') },     // Earthy sienna plate body
        uBarkGolden: { value: new THREE.Color('#9c7753') },   // Weathered tan-cork ridge
        uBarkSunlit: { value: new THREE.Color('#c7a882') },   // Sunlit dry lichen crest
        uMossColor: { value: new THREE.Color('#2d571e') },    // Natural crevice moss
      },
