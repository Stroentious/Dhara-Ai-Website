...
export default function Environment({ progressRef, cursorRef, quality = 'high', isBW = false }) {
  ...
  const soilMat = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBioGlow: { value: new THREE.Color('#22c55e') },
      },
      transparent: true,
      depthWrite: false,
  ...
  const dustMat = useMemo(() => {
    return new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uIsLight: { value: isBW ? 1.0 : 0.0 },
        uParticleColor: { value: isBW ? new THREE.Color('#4a8553') : new THREE.Color('#86efac') },
      },
      transparent: true,
      depthWrite: false,
  ...
  return (
    <group>
      ...
      {/* Sky / Foliage Cool Fill Bounce Light */}
      <directionalLight
        position={[-4.5, 4.2, -3.2]}
        intensity={isBW ? 0.75 : 0.95}
        color={isBW ? '#a7f3d0' : '#2dd4bf'}
      />

      {/* Subterranean Biological Root-Zone Point Light */}
      <pointLight
        position={[0, -0.45, 0.35]}
        intensity={isBW ? 0.9 : 1.4}
        distance={4.5}
        decay={2}
        color="#22c55e"
      />
