...
const BRANCH_VERTEX_SHADER = /* glsl */`
  ...
  void main() {
    ...
    // ── BOTANICAL BRANCH TAPERING & SHOULDER FLARE ──
    // Junction flare near branch attachment (aProg < 0.15)
    float junctionFlare = max(0.0, 1.0 - aProg * 6.6) * 0.35;
    float taper = (1.0 - aProg * 0.72) + junctionFlare;
    float currentRadius = aBaseRadius * taper * localExtrude;
...
const BRANCH_FRAGMENT_SHADER = /* glsl */`
  ...
  void main() {
    ...
    // Normal perturbation
    vec3 bumpedNormal = computeBranchBumpedNormal(vWorldNormal, vWorldPos, barkHeight, 0.038);

    // ── 2. DIRECTIONAL SUN & AMBIENT ILLUMINATION ──
    ...
    float creviceAO = clamp(grain * 0.65 + 0.35, 0.0, 1.0);
    creviceAO *= clamp(bumpedNormal.y * 0.30 + 0.70, 0.0, 1.0);
    ...
    vec3 woodColor = mix(shadowTone, midTone, clamp(diffuse * 1.4, 0.0, 1.0));
    woodColor = mix(woodColor, crestTone, smoothstep(0.55, 1.0, diffuse) * 0.45);

    vec3 litWood = woodColor * (diffuse * 0.85 + 0.20) * creviceAO;
    litWood += uBarkSunlit * max(0.0, NdotL) * 0.18;

    // Specular highlight on wood curve
    vec3 halfDir = normalize(sunDir + viewDir);
    float NdotH = max(0.0, dot(bumpedNormal, halfDir));
    float specular = pow(NdotH, 16.0) * 0.14 * (1.0 - (1.0 - creviceAO) * 0.6);
    litWood += vec3(0.94, 0.92, 0.88) * specular;

    // Rim light for depth separation against foliage and background
    float rim = pow(1.0 - max(0.0, dot(bumpedNormal, viewDir)), 3.0) * 0.12;
    litWood += uBarkSunlit * rim;

    // ── 4. GROWING TIP CAMBIUM BLEND (Tender green vegetative apex) ──
    float tipFactor = smoothstep(0.82, 1.0, vProg);
    vec3 finalCol = mix(litWood, uTipColor, tipFactor * 0.55);
  ...
  const { branchGeometries, branchMaterial } = useMemo(() => {
    ...
    const mat = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        uBarkDeep: { value: new THREE.Color('#120c08') },
        uBarkDark: { value: new THREE.Color('#2c1c13') },
        uBarkMid: { value: new THREE.Color('#523a27') },
        uBarkWarm: { value: new THREE.Color('#735338') },
        uBarkGolden: { value: new THREE.Color('#9c7753') },
        uBarkSunlit: { value: new THREE.Color('#c7a882') },
        uTipColor: { value: new THREE.Color('#52be6b') },
      },
